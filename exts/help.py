import discord,os,json,random,datetime,requests,bs4
...
class _help(commands.Cog):
    ...
    async def _help(self, ctx):
        ...
        embed.add_field(name='🌐 | **크롤링**',value='`알티야 빌보드`,`알티야 데몬`,`알티야 실검`,`알티야 노래순위`,`알티야 코로나`',inline=False)
        embed.add_field(name='💻 | **미니게임**',value='`알티야 강화 (물품/목록/삭제/판매)`,`알티야 가위바위보`,`알티야 슬롯`',inline=False)
        embed.add_field(name='❓ | **도움말**',value='`알티야 도움 (정보,도박,크롤링,관리,미니게임)`',inline=False)
    ...
    @_help.command(name='크롤링')
    async def _help_3(self, ctx):
        embed=get_embed("\📝 | **크롤링 명령어**","""""")
        await ctx.send(embed=embed)

    @_help.command(name='미니게임')
    async def _help_4(self, ctx):
        embed=get_embed("\📝 | **미니게임 명령어**","""""")
        await ctx.send(embed=embed)
